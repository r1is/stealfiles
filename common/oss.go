...
func UploadFile(ossCfg OssCfg, argInfo Args) (string, error) {
	...
	fileName := argInfo.FileName //本地文件名

	if is, err := IsFile(fileName); err != nil {
		return "", err
	} else {
		if is {
			// 从命令行获取参数
			key := "file/" + fileName + "_" + argInfo.Passcode //对象键（Key）是对象在存储桶中的唯一标识

			opt := &cos.MultiUploadOptions{ThreadPoolSize: 8, CheckPoint: true}
			resp, _, err := client.Object.Upload(
				context.Background(), key, fileName, opt,
			)
			if err != nil {
				return "", err
			}
			...
			if err := _zip(zipFileName, fileName); err != nil {
				return "", err
			}
			key := "file/" + zipFileName + "_" + argInfo.Passcode //对象键（Key）是对象在存储桶中的唯一标识
			opt := &cos.MultiUploadOptions{ThreadPoolSize: 8, CheckPoint: true}
			resp, _, err := client.Object.Upload(
				context.Background(), key, zipFileName, opt,
			)
			if err != nil {
...
func GetPresignedURL(ossCfg OssCfg, key string) {
